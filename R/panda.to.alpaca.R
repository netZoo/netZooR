...
#'
#' @param panda.net1 data.frame indicating an complete network of one condition generated by \code{\link{panda.py}}
#' @param panda.net2 data.frame indicating an complete network of another condition generated by \code{\link{panda.py}}
#' @param file.stem String indicating the folder path and prefix of result files, where all results will be stored. The default value is NULL.
#' @param verbose Boolean vector indicating whether the full differential modularity matrix should also be written to a file. The default value is 'FALSE'.
#'
...
#' 
#' @export
panda.to.alpaca <- function(panda.net1, panda.net2, file.stem = NULL, verbose = F){
  # remove "Motif" column
  ...
  if (!is.null(file.stem)){message(paste("the ALPACA output located in", file.stem, sep=""))}
  return(alp)
  }
