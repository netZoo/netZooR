...
test_that("panda function works", {
  ...
  # test 9: check if PANDA result is correct when processMode = union, precision = double
   ...
   # test 16: check if PANDA result is correct when modeProcess = intersection, and all the rest arguments are opposite to the default values:
   # i.e computing = "cpu", save_memory =T , precision="single", save_memory = F, save_tmp=F, keep_expression_matrix = T, modeProcess = 'union'
   ...
   # test 17: when processMode = legacy, remove_missing=FALSE
   ...
   test4Panda <- panda.py(T4_expression_file_path, motif_file_path,ppi_file_path, modeProcess = "legacy", remove_missing = TRUE)$panda
