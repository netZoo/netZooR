...
test_that("lioness function works", {
  ...
  expect_error(lioness())
  
  # test error when not provide prior motif data
  expect_error(test1 <- lioness(T4_expression_file_path))
  ...
  expect_message(test2 <- lioness(T4_expression_file_path,motif_file_path,rm_missing = FALSE, start_sample=1, end_sample=1),"")
  
  # run LIONESS with T4 dataset for only sample 1s
  actual_T4lioness<- lioness(T4_expression_file_path, motif_file_path,ppi_file_path, rm_missing = TRUE,start_sample=1, end_sample=1)
