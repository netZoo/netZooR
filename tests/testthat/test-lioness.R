...
test_that("lioness function works", {
  ...
  expect_error(test1 <- lioness.py(T4_expression_file_path))
  ...
  #expect_message(test2 <- lioness.py(T4_expression_file_path,motif_file_path,rm_missing = FALSE, start_sample=1, end_sample=1),"")
  
  # run LIONESS with T4 dataset for only sample 1s
  actual_T4lioness<- lioness.py(T4_expression_file_path, motif_file_path,ppi_file_path, rm_missing = TRUE,start_sample=1, end_sample=1)
