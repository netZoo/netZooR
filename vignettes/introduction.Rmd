...
Or:
`subtypes <- sambar(mutdata=mut.ucec, esize=exon.size, signatureset=system.file("extdata", "h.all.v6.1.symbols.gmt", package="SAMBAR", mustWork=TRUE), cangenes=genes, kmin=2, kmax=4)`
...
Please note that, in order to be able to cluster the data using the binomial distance, SAMBAR removes samples that have mutation scores of 0 across all pathways, and pathways that have mutation scores of 0 across all samples. Using fewer genes of interest in `cangenes` or fewer pathways could result in errors. We have not yet added user-friendly error checks to the package.
